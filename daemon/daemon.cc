...
int Daemon::daemonStart(const char *pidfile) {
    ...
    if (pid > 0) {
       ...
       //EPERM EPERM 权限不够无法传送信号给指定进程
       if (errno == EPERM) {
           fprintf(stderr, "no EPERM sig to daemon\n");
           return -1;
       }
       if (errno == ESRCH) {
           fprintf(stderr, "daemon not exists, continue\n");
       }
    }

    if (getppid() == 1) {
       fprintf(stderr, "already exists daemon parent, can't start\n");
       return -1;
    }
